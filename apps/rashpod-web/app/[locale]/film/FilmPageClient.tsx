...
function FilmContent({ initialFilms = [], initialDesigners = [], initialCategories = [] }: { initialFilms?: FilmListing[]; initialDesigners?: CatalogDesigner[]; initialCategories?: CatalogCategory[] }) {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/shop/listings?${params}`, { signal: controller.signal })
      ...
      .catch((requestError) => { if (timedOut || (requestError instanceof Error && requestError.name !== "AbortError")) setError(true); })
      .finally(() => {
  ...
  return (
    <div className="mx-auto max-w-storefront px-4 py-7 sm:px-6 md:py-10">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <Link href="/film/custom" className="inline-flex min-h-11 items-center rounded-pill bg-brand-peach px-5 text-sm font-semibold text-brand-ink focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-peach/25">{filmT("customFilm")}</Link>
      </div>
      ...
      <div className="grid items-start gap-7 lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        <section className="min-w-0" aria-label={filmT("title")}>
