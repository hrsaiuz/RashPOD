...
import { Card, EmptyState, ErrorState, Button } from "@rashpod/ui";
import { Users } from "lucide-react";
import type { DesignerSummary } from "../../../lib/catalog";
...
export default function DesignersPageClient({
  ...
  const designers = initialDesigners;
              ...
              <Card variant="lift" className="p-6 h-full group">
                <Link href={`/designer/${designer.handle}`}>
                  ...
                  <p className="text-sm text-brand-muted mb-3">
                    {designer.listingsCount}{" "}
                    {designer.listingsCount === 1 ? "listing" : "listings"}
                  </p>
...
export function DesignersPageError({ reset }: { reset?: () => void }) {
  const router = useRouter();
