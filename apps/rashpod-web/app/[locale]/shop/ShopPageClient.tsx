...
function ShopContent({
  ...
  initialDesigners?: Array<{ handle: string; displayName: string }>;
}) {
  ...
  const [availableDesigners, setAvailableDesigners] = useState<Array<{ handle: string; displayName: string }>>(initialDesigners);
  ...
  useEffect(() => {
    if (initialDesigners.length) return;
  ...
  }, [apiBase, initialDesigners.length]);
  ...
  const SidebarContent = () => (
    <div className="space-y-6">
      <div>
        <h3 id="categories" className="text-sm font-semibold text-brand-ink mb-3 scroll-mt-28">Category</h3>
