...
export default function CheckoutPage() {
  ...
  const canContinue = items.length > 0 && authState === "authed";
  ...
  async function placeOrder() {
    ...
    try {
      ...
      const order = await api.post<Order>("/orders", {
        ...
        deliveryZone: "UZ",
        notes: `Web checkout. Address: ${selectedAddress}. Shipping: ${selectedShipping}.`,
      });
      const payment = await api.post<{ paymentId: string; checkoutUrl?: string }>("/payments/click/create", { orderId: order.id });
      clearCart();
      window.location.href = `/checkout/success?orderId=${encodeURIComponent(order.id)}&paymentId=${encodeURIComponent(payment.paymentId)}`;
    } catch (err) {
      ...
      <div className="mx-auto max-w-[1232px] px-5 pb-16 pt-4">
        ...
        <div className="mt-10 grid gap-16 lg:grid-cols-[1fr_470px]">
          <section>
            ...
            {items.length === 0 && authState === "authed" ? (
            ...
            {items.length > 0 ? (
              <>
                {step === "address" ? <AddressStep selected={selectedAddress} onSelect={setSelectedAddress} /> : null}
...
async function syncLocalCartToServer(items: CartItem[]) {
  ...
  for (const item of items) {
    await api.post("/cart", { listingId: item.listingId, quantity: item.quantity });
  }
}
...
function AddressStep({ selected, onSelect }: { selected: string; onSelect: (value: string) => void }) {
  return (
    <div className="mt-12 max-w-[650px]">
      ...
      />
      <button className="mt-7 inline-flex items-center gap-4 text-[16px] text-brand-blue">
  ...
  return (
    <aside className="rounded-[10px] bg-brand-bg p-8 shadow-[8px_10px_16px_rgba(0,0,0,0.25)]">
      ...
      <div className="max-h-[430px] space-y-3 overflow-y-auto pr-1">
        {items.length === 0 ? <div className="rounded-[8px] bg-white p-8 text-center text-brand-muted">Your cart is empty.</div> : null}
        {items.map((item) => (
